...
try:
    from src.mappings.prodcom_waste import (
        PRODCOM_TO_EWC,
    ...
    from src.mappings.ewc_stat import (
        EWC_STAT_CODES,
        get_ewc_description,
    )
    from src.loaders.io import INTERIM_DIR, PROCESSED_DIR
    ...
    from ewc_stat import EWC_STAT_CODES, get_ewc_description
...
def fetch_prodcom_data(
    ...
    """
    Fetch PRODCOM data from Eurostat using the eurostat package.
    ...
    # Fetch data using eurostat package
    print(f"Fetching PRODCOM data from Eurostat ({PRODCOM_DATASET_ID})...")
    df = eurostat.get_data_df(PRODCOM_DATASET_ID.lower(), flags=False)

    if df.empty:
        print("No data returned from Eurostat")
        return pd.DataFrame()

    # Standardize column names - fix backslash in indicator column
    df.columns = df.columns.str.lower()
    df.columns = [c.split('\\')[0] if '\\' in c else c for c in df.columns]

    # Identify year columns (numeric column names)
    year_cols = [c for c in df.columns if str(c).isdigit()]
    year_cols_in_range = [c for c in year_cols if start_year <= int(c) <= end_year]

    if not year_cols_in_range:
        print(f"No data found for years {start_year}-{end_year}")
        return pd.DataFrame()

    # Filter by indicators early (before melt) for efficiency
    indicators_col = 'indicators' if 'indicators' in df.columns else None
    if indicators_col:
        df = df[df[indicators_col].isin(indicators)]

    # Filter by countries (reporter column)
    reporter_col = 'reporter' if 'reporter' in df.columns else 'geo'
    if reporter_col in df.columns:
        df = df[df[reporter_col].isin(countries)]
        if reporter_col != 'reporter':
            df = df.rename(columns={reporter_col: 'reporter'})

    # Filter by products
    product_col = 'prccode' if 'prccode' in df.columns else 'product'
    if product_col in df.columns and products:
        df = df[df[product_col].astype(str).isin(products)]
    if product_col in df.columns and product_col != 'product':
    ...
    print(f"Loaded {len(df)} rows of PRODCOM data")
    return df
...
def map_prodcom_to_waste(
    ...
    if mapping:
        ...
        if factor > 0:
            ...
            from src.mappings.ewc_stat import NACE_TYPICAL_WASTES
