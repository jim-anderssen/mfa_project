...
class GVAAllocator:
    ...
    def allocate_waste(
        ...
        grouped = wasgen_df.groupby(['country', 'nace', 'waste', 'year']).agg({
            'tonnes': 'sum'
        }).reset_index()
        ...
        for _, row in grouped.iterrows():
            country, nace, waste, year = (
                row['country'], row['nace'], row['waste'], row['year']
            )
    ...
    def _standardize_wasgen_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'country' in df.columns:
            ...
            sample = df['country'].dropna().iloc[0] if len(df['country'].dropna()) > 0 else ''
            if len(sample) > 2:
    ...
    def get_allocation_summary(self, allocated_df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary.columns = [
            ...
            'n_companies', 'mean_share', 'std_share'
        ]

        if len(self.unallocated) > 0:
            unalloc_by_nace = self.unallocated.groupby(
                ['country', 'nace']
            )['national_tonnes'].sum()
...
def run_gva_allocation_pipeline(
    ...
    # Create allocator
    allocator = load_gva_allocator(
        Path(companies_path),
        filter_nace=filter_nace
    )
    ...
    allocated = allocator.allocate_waste(
        wasgen,
        countries,
        deduplicate_nace=deduplicate_nace
    )
